...
const EXTERNAL_TIMEOUT_MS = Number(
  process.env.EXTERNAL_JOB_TIMEOUT_MS ?? 10 * 60_000,
);
...
export class ReplicateService {
  ...
  async makeGif(imageUrl: string): Promise<string> {
    const cleaned = await this.cleanImage(imageUrl);
    const mp4 = await this.animateImage(cleaned);
    return this.convertVideoToGif(mp4);
  }
  ...
  async cleanImage(src: string): Promise<string> {
  ...
  async animateImage(img: string): Promise<string> {
    const out = await withTimeout(
      this.replicate.run('bytedance/seedance-1-lite', {
        input: {
          fps: 24,
          image: img,
          prompt: 'Animate',
  ...
  async convertVideoToGif(mp4Url: string): Promise<string> {
    ...
    while (!gifUrl) {
      if (Date.now() >= deadline)
        throw new InternalServerErrorException('GIF conversion timed out');
